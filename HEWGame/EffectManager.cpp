...
void EffectManager::Init()
{
    ...
    {
    EffectType::Dash,               //エフェクトの種類
    16,                             //総コマ数 (全部で何コマあるか)
    4,                              //横の分割数 (横に何コマ並んでいるか)
    640.0f,                         //1コマの幅 (TexW)
    320.0f,                         //1コマの高さ (TexH)
    0.05f,                          //アニメーション速度 (1コマの秒数)
    0.0f,                           //発生位置の横ズレ (OffsetX: プレイヤー中心から横にどれだけずらすか)
    0.0f,                           //発生位置の縦ズレ (OffsetY: 足元に合わせるための調整)
    0.5f,                           //サイズ倍率 
    640.0f,                         //画像の読み取り開始位置X (StartTexX: 画像の右側を使うため320から開始)
    0.0f                            //画像の読み取り開始位置Y (StartTexY)
    };
    m_EffectDefs[EffectType::DashIdol] =
    {
       EffectType::DashIdol,          //エフェクトの種類
       32,							   //総コマ数 (全部で何コマあるか)
       8,                            //横の分割数 (横に何コマ並んでいるか)
       320.0f,                        //1コマの幅 (TexW)
       320.0f,                        //1コマの高さ (TexH)
       0.03f,                         //アニメーション速度 (1コマの秒数)
       300.0f,                        //発生位置の横ズレ (OffsetX: プレイヤー中心から横にどれだけずらすか)
       0.0f,                          //発生位置の縦ズレ (OffsetY: 足元に合わせるための調整)
       0.9f,                          //サイズ倍率
       320.0f,                          //画像の読み取り開始位置X (StartTexX: 画像の右側を使うため320から開始)
       0.0f                           //画像の読み取り開始位置Y (StartTexY)
    };
    m_EffectDefs[EffectType::Attack] =
    {
       EffectType::Attack,          //エフェクトの種類
       6,							   //総コマ数 (全部で何コマあるか)
       6,                            //横の分割数 (横に何コマ並んでいるか)
       320.0f,                        //1コマの幅 (TexW)
       320.0f,                        //1コマの高さ (TexH)
       0.03f,                         //アニメーション速度 (1コマの秒数)
       70.0f,                        //発生位置の横ズレ (OffsetX: プレイヤー中心から横にどれだけずらすか)
       0.0f,                          //発生位置の縦ズレ (OffsetY: 足元に合わせるための調整)
       0.9f,                          //サイズ倍率
       320.0f,                          //画像の読み取り開始位置X (StartTexX: 画像の右側を使うため320から開始)
       0.0f                           //画像の読み取り開始位置Y (StartTexY)
