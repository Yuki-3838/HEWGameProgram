...
void Animator::Init(int count, int xCount, float w, float h, float durationPerFrame, float offsetY, bool loop,float renderOffsetX, float renderOffsetY, float scale)
{
...
{
    // データがない場合の安全対策
    if (m_frames.empty()) return { 0,0,0,0,0,0,0,1.0f };
