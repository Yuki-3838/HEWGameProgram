...
#include <Xinput.h> //XInput(XBOX規格のコントローラー)を使うためのヘッダーファイル
#pragma comment (lib, "xinput.lib") //XInputを使うために必要

#include<Dinput.h> //Dinput(PSのコントローラ等)を使うためのヘッダーファイル
#pragma comment(lib, "dinput8.lib")
#pragma comment(lib, "dxguid.lib") //DInputを使うために必要
...
private:
	...
	//XInputコントローラー入力情報を保存する変数
	XINPUT_STATE controllerState = {};
	XINPUT_STATE controllerState_old = {};

	//Dinputのコントローラー用
	IDirectInput8* m_pDI = nullptr; // DirectInput 本体
	IDirectInputDevice8* m_pKeyboard = nullptr; // キーボードデバイス
